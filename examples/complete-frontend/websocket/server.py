...
class WebSocketServer:
    ...
    def __init__(self, host='0.0.0.0', port=8086, log_level='INFO', log_file=None, debug=False):
        ...
        self.logger.info(f"WebSocket server initialized on ws://{self.host}:{self.port}")
    ...
    def init_edpmt(self):
        """Initialize EDPMT client."""
        try:
            self.logger.info("Initializing EDPMT client...")
            config = {'hardware_simulators': Config.USE_HARDWARE_SIMULATORS}
            self.edpmt_client = EDPMTransparent(config=config)
            self.logger.info("EDPMT client initialized successfully.")
        except Exception as e:
            self.logger.error(f"Failed to initialize EDPMT client: {e}. Running in simulation mode.")
    ...
    async def start_edpmt_client(self):
        ...
        try:
            self.logger.info("Starting EDPMT client...")
            await self.edpmt_client.start_server()
            self.logger.info("EDPMT client started successfully.")
        except Exception as e:
            self.logger.error(f"Failed to start EDPMT client: {e}. Running in simulation mode.")
    ...
    async def websocket_handler(self, request):
        ...
        if origin and origin.rstrip('/') not in [o.rstrip('/') for o in allowed_origins]:
            self.logger.warning(f"Rejected WebSocket connection from unauthorized origin: {origin}")
        ...
        self.logger.info(f"WebSocket connection established. Total connections: {len(self.websockets)}")
        ...
        try:
            ...
            self.logger.info(f"WebSocket connection closed. Remaining connections: {len(self.websockets)}")
    ...
    async def execute_handler(self, request):
        ...
        try:
            ...
            self.logger.exception("/api/execute failed")
            return web.json_response({'error': str(e)}, status=500)
    ...
    async def run(self):
        ...
        self.logger.info("✅ WebSocket server is running. Press Ctrl+C to stop.")
        try:
            while True:
